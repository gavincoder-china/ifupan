...
class MindMapDAO:
    @staticmethod
    async def create(db: Session, input_text: str, prompt_type: str, mind_map_file: str, pdf_file: str):
        db_item = MindMap(input_text=input_text, prompt_type=prompt_type, mind_map_file=mind_map_file, pdf_file=pdf_file)
        db.add(db_item)
        await db.commit()
