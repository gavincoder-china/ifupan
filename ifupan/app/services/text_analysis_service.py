...
class TextAnalysisService:
    @staticmethod
    async def analyze_and_save(db: AsyncSession, input_text: str, prompt_type: str):
    ...
    @staticmethod
    async def get_all_analyses(db: AsyncSession, skip: int = 0, limit: int = 100):
        return await TextAnalysisDAO.get_all(db, skip, limit)

    @staticmethod
    async def analyze(db: AsyncSession, text: str, prompt_type: str):
