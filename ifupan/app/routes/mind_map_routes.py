...
def generate_mind_map():
    ...
    result = asyncio.run(MindMapService.generate_and_save(next(get_db()), text, prompt_type))
...
def get_mind_map(mind_map_id):
    result = asyncio.run(MindMapService.get_mind_map_by_id(next(get_db()), mind_map_id))
    if result:
...
def get_all_mind_maps():
    results = asyncio.run(MindMapService.get_all_mind_maps(next(get_db())))
