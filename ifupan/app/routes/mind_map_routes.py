...
def generate_mind_map():
    for db in get_db():
        ...
        result = MindMapService.generate_and_save(db, text, prompt_type)
...
def get_mind_map(mind_map_id):
    for db in get_db():
        result = MindMapService.get_mind_map_by_id(db, mind_map_id)
        if result:
...
def get_all_mind_maps():
    for db in get_db():
        results = MindMapService.get_all_mind_maps(db)
